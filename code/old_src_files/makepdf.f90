...
!#######################################################################################
subroutine set_quake_pdf(pdf,quake_pdf,amesh)
  type(pdfinputs) :: pdf
  type(mesh) :: amesh
  ...
  allocate(quake_pdf(amesh%Ncells))
  quake_pdf = pdf%g_pdf
...
quake_pdf = 1./float(amesh%Ncells-1)

end subroutine uniform_pdf
!#######################################################################################
subroutine calc_rupt_front(amesh)
  use LAT_time
  use calc_time 
  implicit none
...
!#######################################################################################
 subroutine pdftoslip(model,amesh,surface,pdf_type,quake_pdf,output_level,dist2)
  use typedef
  use LAT_source
  use LAT_time
  use distance 
...
! pdf on cells
  real(pr), dimension(amesh%Ncells) :: quake_pdf
  real(pr) :: rmin,rmax
  real(pr) :: random
...
if (abs(maxval(quake_pdf)-minval(quake_pdf)) <= epsilon(1.)) then
  idxmax = ceiling(random*float(amesh%Ncells))  ! random choice
...
! initializing the slip array
  allocate(slip(amesh%Ncells))
  allocate(slip_prim(amesh%Ncells))
  allocate(slip_sec(amesh%Ncells))
     ...
     if (i == 1) then
          ...
          do while (r(i) > db)
            ...
            if (cnt == amesh%Ncells+1 ) then
             ...
             call random_number(random)
             cellidx = ceiling(random*float(amesh%Ncells))  ! random choice
          enddo
     else
...
! primary asperity
     do j=1,amesh%Ncells
        dmean=0._pr
        do k=1,3
           dmean=dmean+dist(casp,amesh%cell(j,k))
        enddo
        dmean=dmean/3._pr
...
! total seismic moment is based on the sum of the moments of all the cells
  ...
  do i=1,amesh%Ncells
     ...
     ave_slip = ave_slip + slip(i)*area(i)
