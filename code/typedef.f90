...
!==============================================================================
! subroutine surface_nodes(amesh,surface)
!   use LAT_source 
!   use LAT_mesh 
...
!       surface%NoSurfNodes = nlines
!       allocate(surface%SurfNodes(surface%NoSurfNodes))
!       do i = 1, surface%NoSurfNodes
!         read(13,*) surface%SurfNodes(i)
!       enddo
!       close (13)
...
!       surface%NoSurfNodes = nlines
!       allocate(surface%SurfNodes(surface%NoSurfNodes))
!       surface%SurfNodes = surface_mask(1:nlines)
...
!     do i = 1,surf%NoSurfNodes
!       not_on_boundary = .true.
!       do j = 1,amesh%Ncells
!         if (any(surface%SurfNodes(i).eq.amesh%cell(j,:))) then
...
!          write(*,*) 'ERROR: Node not on mesh edge:', surf%SurfNodes(i)
!          stop
!       endif
!     enddo
!   endif
...
type(mesh) :: amesh
integer :: i
real(pr),dimension(3) :: x,y,z,vec_ab,vec_ac
real(pr) :: ab,ac,res,theta
real(pr) :: tot_area
allocate(area(amesh%Ncells))
...
do i = 1,amesh%Ncells
  ...
  theta = acos(res)
  area(i) = ab*ac*sin(theta)/float(2)
...
!==============================================================================
! subroutine  select_fault_zone_global(amesh,quake, model,z0,pdf)
subroutine  select_fault_zone_global(amesh,quake, model,pdf)
  ...
  integer(pin),allocatable,dimension(:) :: quake_nodes,quake_cells,haveit,potential_cells
  !real,allocatable,dimension(:) :: pdf
  integer(pin) :: cell_options(3),c_nodes(3),test_cell(3)
  real(pr) :: vec(3), curcum
  real(pr) :: random , quake_area,prob_lr, prob_ud
  integer(pin) :: get_out, inodes ,icells, c_cell ,i,iu_cells,random_pick
  integer(pin) :: lr_count, ud_count, tot_count, lr_elements(3), ud_elements(3)
	integer(pin) :: counter,ipcells,ii,k,i_use
	real(pr) :: epi_depth,d, ug,sigma,pdfint,min_z,max_z,width_est
  ! real(pr) :: z0   ! depth of free surface
  logical :: find_cell,no_holes,reached_width
  real(pr) :: minv,maxv,set_width,trial_width,dmax
  real(pr) :: numrnd
  real(pr),allocatable,dimension(:) ::dz,dd
  real(pr), allocatable,dimension(:) :: weighting
  integer(pin) :: opposite_idx
  integer(pin) :: min_loc,max_loc
  integer(pin) :: nlength
...
! randomly pick starting cell
        ...
        c_cell  = quake%nuc_id   ! use prescribed nucleation cell
...
!====================  add cell to slipping list & remove from potential list
     ...
     quake_area = quake_area + area(quake_cells(icells)) ! assume area is in m^2
     icells = icells+1
...
enddo
 ...
 ! model%actual_width = width_est
  ...
  ! get rid of duplicates
   ...
   allocate(quake%QuakeElem(nlength))
   quake%QuakeElem = quake_cells(1:nlength)
   quake%QuakeElemNo = nlength


   call erasedupli(quake_nodes,inodes-1,nlength)
   allocate(quake%QuakeNodes(nlength))
   quake%QuakeNodes = quake_nodes(1:nlength)
   quake%QuakeNodesNo = nlength
...
!==============================================================================!==============================================================================
! subroutine  select_fault_zone_element(amesh,quake,target_area) ! possibly not needed -> weighted for depth??
!   use LAT_source
!   type(mesh),intent (inout) :: amesh
!   type(source),intent (out) :: quake

!   integer(pin),allocatable,dimension(:) :: quake_nodes,quake_cells,haveit,unused_cells
!   real(pr), intent(in) :: target_area
!   integer(pin) :: cell_options(3),c_nodes(3)
!   real(pr) :: vec(3)
!   real(pr) :: random, alpha , quake_area,prob_lr, prob_ud
!   integer(pin) :: get_out, inodes ,icells, c_cell ,i,iu_cells,random_pick
!   integer(pin) :: lr_count, ud_count, tot_count, lr_elements(3), ud_elements(3)
! 	integer(pin) :: counter
!   logical :: find_cell
...
!                 if (mz(cell_options(i)) >= minval(vec).and.     &
!                     mz(cell_options(i)) <= maxval(vec))   then  ! left/right
!                    lr_count = lr_count+1
...
!   ! Save section of fault used for earthquake
!   allocate(quake%QuakeElem(icells-1),quake%QuakeNodes(inodes-1))
!   quake%QuakeElem = quake_cells(1:icells-1)
!   quake%QuakeNodes = quake_nodes(1:inodes-1)
!   quake%QuakeElemNo = icells-1
!   quake%QuakeNodesNo = inodes-1

!   write(*,*) 'number of elements in slipping zone',quake%QuakeElemNo
!   write(*,*) 'number of nodes in slipping zone',quake%QuakeNodesNo
...
!==============================================================================
subroutine  select_fault_zone(amesh,quake,target_area)
  use LAT_source
  use LAT_mesh_util
...
real(pr) :: vec(3)
real(pr) :: nuc_x,nuc_y,nuc_z
real(pr) :: quake_area
real(pr) :: random, alpha
...
! check to see if element is already in quake_ameshents
                    ...
                    if (haveit(1) == -1) then      ! don't have element
                           if (inx == 1) then   ! initially include all surrounding elements
                             ...
                             quake_area = quake_area + area(quake_ameshents(ele_inx)) ! assume area is in m^2
                             ele_inx = ele_inx+1;
                           else
! ascertain if element is above/below or left/right of current element : shane
                               ...
                               if (mz(choice_amesh(i)) >= minval(vec).and.     &
                                   mz(choice_amesh(i)) <= maxval(vec))   then  ! left/right
                                  lr_count = lr_count+1
...
! Save section of fault used for earthquake
allocate(quake%QuakeElem(ele_inx-1),quake%QuakeNodes(inx-1))
quake%QuakeElem = quake_ameshents(1:ele_inx-1)
quake%QuakeNodes = quake_nodes(1:inx-1)
quake%QuakeElemNo = ele_inx-1
quake%QuakeNodesNo = inx-1
deallocate(quake_nodes,quake_ameshents)


end subroutine select_fault_zone
!==============================================================================
subroutine find_boundary(amesh,quake,surface)
  ! subroutine find_boundary(amesh,quake,surf)
use LAT_source 
use LAT_mesh_util
use LAT_time
type(mesh),intent (inout) :: amesh
...
int = 0
node_idx = 0
allocate(bc_nodes(amesh%Nnodes),int_amesh(amesh%Ncells),bc_amesh(quake%QuakeElemNo))

bc_nodes = 0
bc_amesh = 0
do i = 1,quake%QuakeElemNo
  ! ameshs = neighbouring elements to QuakeElem(i)
       ameshs = amesh%EToE(quake%QuakeElem(i),:)
       call isamember(ameshs,quake%QuakeElem,id) !check if adjoining elements are in defined slipping zone
       if (id(1) == 0) then
          ameshs(1) = quake%QuakeElem(i) ! elements is outside defined fault
       elseif (id(2) == 0) then
          ameshs(2) = quake%QuakeElem(i) ! elements is outside defined fault
       elseif (id(3) == 0) then
          ameshs(3) = quake%QuakeElem(i) ! elements is outside defined fault
       endif

       ! if ameshs contains an element = QuakeElem(i) it is a boundary point
       call find(ameshs,'==',quake%QuakeElem(i),haveit)

       if (haveit(1) == -1) then  ! is interior elements
              int = int+1
              int_amesh(int) = quake%QuakeElem(i)
         ...
         ! check to see if element is part of surface
            ...
            ! if (surf%exists) then
              if (surface%present) then
               do j = 1,3
                 if (any(amesh%cell(quake%QuakeElem(i),j).eq.surface%nodes) ) then
                     not_surface =.false.
                 endif
                enddo
              endif
            if (not_surface) then
              bc_int = bc_int+1
              bc_amesh(bc_int) = quake%QuakeElem(i)
!===== check faces for which nodes make up boundary======
! global nodal id's of current element
              i_nodes = amesh%cell(quake%QuakeElem(i),:)
              if (allocated(idx)) deallocate(idx)
! find neighboring elements
              call find(ameshs,'/=',quake%QuakeElem(i),idx)
...
! remove duplicates
call erasedupli(bc_nodes,quake%QuakeNodesNo,nlength)

quake%QuakeBorder_NodesNo = nlength-1       ! number of vertices that make up boundary of fault

allocate(quake%QuakeBorder_Nodes(quake%QuakeBorder_NodesNo))
quake%QuakeBorder_Nodes = bc_nodes(1:quake%QuakeBorder_NodesNo)    ! vercties that make up boundary of fault
quake%QuakeBorder_elemNo = bc_int     ! number of elements that make up boundary
!amesh%QuakeBorder_elemNo = elem_idx     ! number of elements that make up boundary

allocate(quake%QuakeBorder_elem(quake%QuakeBorder_elemNo))
quake%QuakeBorder_elem = bc_amesh(1:bc_int)
!amesh%QuakeBorder_elem = bc_amesh(1:elem_idx)

end subroutine find_boundary
!==============================================================================
!###############################################################################
subroutine multisource(amesh,quake)
  use LAT_source
  use distance
    implicit none
    type(mesh) :: amesh
  ...
  !initialise array
  allocate(Dist2Border(amesh%Nnodes))
  Dist2Border=infinity
  ...
  do i=1,quake%QuakeBorder_NodesNo
       Dist2Border(quake%QuakeBorder_Nodes(i))=10000._pr   
  enddo
  ! calculate distance to boundary
  fast = .false.
  call onevsall2d(amesh,Dist2Border,fast)
   
  Dist2Border = Dist2Border-10000._pr      !
